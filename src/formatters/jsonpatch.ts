...
interface JSONFormatterContext extends BaseFormatterContext {
    result: Op[]
    path: (string | number)[]
    pushCurrentOp: (obj: { op: 'add'; value: unknown } | { op: 'remove' } | { op: 'replace'; value: unknown }) => void
...
class JSONFormatter extends BaseFormatter<JSONFormatterContext, Op[]> {
    ...
    nodeBegin({ path }: JSONFormatterContext, _key: string, leftKey: string | number) {
        path.push(leftKey)
    }
    ...
    format_node(context: JSONFormatterContext, delta: ObjectDelta | ArrayDelta, left: unknown) {
