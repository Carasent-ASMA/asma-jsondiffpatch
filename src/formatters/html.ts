...
class HtmlFormatter extends BaseFormatter<HtmlFormatterContext> {
    ...
    rootBegin(context: HtmlFormatterContext, type: DeltaType, nodeType: NodeType) {
    ...
    format_unchanged(context: HtmlFormatterContext, _delta: undefined, left: unknown) {
    ...
    format_movedestination(context: HtmlFormatterContext, _delta: undefined, left: unknown) {
        if (typeof left === 'undefined') {
            return
        }
        context.out('<div class="jsondiffpatch-value">')
    ...
    format_node(context: HtmlFormatterContext, delta: ObjectDelta | ArrayDelta, left: unknown) {
        // recurse
...
const adjustArrows = function jsondiffpatchHtmlFormatterAdjustArrows(nodeArg?: Element) {
    const node = nodeArg || document
    const getElementText = ({ textContent, innerText }: HTMLDivElement) => textContent || innerText
    const eachByQuery = (el: Element | Document, query: string, fn: (element: HTMLElement) => void) => {
        const elems = el.querySelectorAll(query)
        for (let i = 0, l = elems.length; i < l; i++) {
            fn(elems[i] as HTMLElement)
        }
    }
    const eachChildren = ({ children }: ParentNode, fn: (child: Element, index: number) => void) => {
...
export const showUnchanged = (show?: boolean, node?: Element | null, delay?: number) => {
...
export const hideUnchanged = (node?: Element, delay?: number) => showUnchanged(false, node, delay)
