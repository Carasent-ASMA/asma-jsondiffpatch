...
class ConsoleFormatter extends BaseFormatter<ConsoleFormatterContext> {
    ...
    rootBegin(context: ConsoleFormatterContext, type: DeltaType, nodeType: NodeType) {
    ...
    rootEnd(context: ConsoleFormatterContext, type: DeltaType, nodeType: NodeType) {
        if (type === 'node') {
            context.indent(-1)
            context.out(nodeType === 'array' ? ']' : '}')
        }
    ...
    format_unchanged(context: ConsoleFormatterContext, _delta: undefined, left: unknown) {
    ...
    format_movedestination(context: ConsoleFormatterContext, _delta: undefined, left: unknown) {
        if (typeof left === 'undefined') {
            return
        }
        this.formatValue(context, left)
    ...
    format_node(context: ConsoleFormatterContext, delta: ObjectDelta | ArrayDelta, left: unknown) {
        // recurse
