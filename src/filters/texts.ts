...
import type { AddedDelta, DeletedDelta, Filter, ModifiedDelta, MovedDelta, Options, TextDiffDelta } from '../types.js'
...
function getDiffMatchPatch(options: Options | undefined, required: true): DiffPatch
function getDiffMatchPatch(options: Options | undefined, required?: boolean): DiffPatch | null
        ...
        cachedDiffPatch = {
            ...
            patch: function (txt1, patch) {
                ...
                for (let i = 0; i < results[1].length; i++) {
                    if (!results[1][i]) {
                        const error: Error & { textPatchFailed?: boolean } = new Error('text patch failed')
...
export const diffFilter: Filter<DiffContext> = function textsDiffFilter(context) {
    ...
    const minLength =
        (context.options && context.options.textDiff && context.options.textDiff.minLength) || DEFAULT_MIN_LENGTH
...
export const patchFilter: Filter<PatchContext> = function textsPatchFilter(context) {
    if (context.nested) {
        return
    }
...
export const reverseFilter: Filter<ReverseContext> = function textsReverseFilter(context) {
    if (context.nested) {
        return
    }
    const nonNestedDelta = context.delta as AddedDelta | ModifiedDelta | DeletedDelta | MovedDelta | TextDiffDelta
