...
describe('DiffPatcher', () => {
                ...
                describe(name, () => {
                    ...
                    if (!example.noPatch) {
                        it('can patch', function () {
                            const right = instance.patch(jsondiffpatch.clone(example.left), example.delta)
                        ...
                        it('can reverse delta', function () {
                            ...
                            if (example.exactReverse !== false) {
                                ...
                                expect(instance.patch(jsondiffpatch.clone(example.right), reverse)).toEqual(
                                    example.left,
                                )
                        ...
                        it('can unpatch', function () {
                            const left = instance.unpatch(jsondiffpatch.clone(example.right), example.delta)
    ...
    describe('plugins', () => {
        ...
        describe('supporting numeric deltas', () => {
            ...
            it('diff', function () {
                ...
                const numericDiffFilter: jsondiffpatch.Filter<jsondiffpatch.DiffContext> = (context) => {
            ...
            it('patch', function () {
                const numericPatchFilter: jsondiffpatch.Filter<jsondiffpatch.PatchContext> = (context) => {
                    const deltaWithNumericDifference = context.delta as DeltaWithNumericDifference
                    if (
                        ...
                        context
                            .setResult(
                                (context.left as number) + (deltaWithNumericDifference as NumericDifferenceDelta)[1],
                            )
                            .exit()
                    }
                }
                ...
                const delta = {
                    population: [0, 3, NUMERIC_DIFFERENCE] as unknown as jsondiffpatch.Delta,
                }
            ...
            it('unpatch', function () {
                const numericReverseFilter: jsondiffpatch.Filter<jsondiffpatch.ReverseContext> = (context) => {
                    if (context.nested) {
                        return
                    }
                    const deltaWithNumericDifference = context.delta as DeltaWithNumericDifference
        ...
        describe('removing and replacing pipe filters', () => {
            ...
            it('replaces specified filter', function () {
                const fooFilter: jsondiffpatch.Filter<jsondiffpatch.DiffContext> = (context) => {
    ...
    describe('formatters', () => {
        describe('jsonpatch', () => {
            ...
            const expectFormat = (before: unknown, after: unknown, expected: jsonpatchFormatter.Op[]) => {
            ...
            const replaceOp = (path: string, value: unknown): jsonpatchFormatter.ReplaceOp => ({
                op: 'replace',
                path,
                value,
            })
